...
class RDDQNAgent:
  '''
  Recurrent Double DQN agent with LSTM to adjust trading parameters (p and q).
  '''
  ...
  def _build_model(self, learning_rate):
    '''
    Build the LSTM-based neural network for RDDQN.
    '''
    model = Sequential()
    model.add(Input(shape=(self.sequence_length, self.state_size)))
...
def get_state_sequence(df, t, sequence_length, feature_cols):
  '''
  Extract a sequence of states ending at index t.
  '''
...
def simulate_trading(df, agent, episodes=10, fee_open=0.0002, fee_close=0.0002, max_lookahead_seconds=20):
  ...
  for episode in range(episodes):
    print(f'Train Episode {episode + 1} / {episodes}')
    ...
    agent.replay()
    print(f'Total Reward: {total_reward}, Epsilon: {agent.epsilon:.2f}')

def evaluate_agent(df, agent, fee_open=0.0002, fee_close=0.0002, max_lookahead_seconds=20):
  '''
  Evaluate the trained agent's performance with sequences.
  '''
  feature_cols = ['price', 'returns', 'short_ma', 'long_ma', 'volatility', 'macd_hist',
                  'rsi', 'imbalance', 'velocity', 'trend_direction', 'trend_strength', 'amount_z']
...
if __name__ == '__main__':
  ...
  state_size = 12
  action_size = 9
  sequence_length = 60  # Matches window_long from calc_features_trades
  episodes = 50
  ...
  # Create agent
  agent = RDDQNAgent(state_size, action_size, sequence_length=sequence_length)
  ...
  while current_timestamp <= max_timestamp:
    ...
    if not df_features_trades_day.empty:
      print(f'Processing data from {current_timestamp.date()}')
