...
# New function to create sequences for Informer
...
def informer_encoder(inputs, seq_length, head_size, num_heads, ff_dim, dropout=0.3):
  d_model = inputs.shape[-1]  # Get feature dimension from input shape

  # Positional encoding
  pos_encoding = tf.keras.layers.Embedding(seq_length, d_model)(tf.range(seq_length))
  x = inputs + tf.cast(pos_encoding, dtype=inputs.dtype)

  # ProbSparse Attention (approximated with standard attention)
  attn_output = tf.keras.layers.MultiHeadAttention(
    num_heads=num_heads,
    key_dim=head_size,
    attention_axes=1
  )(x, x)
  x = tf.keras.layers.Dropout(dropout)(attn_output)
  x = tf.keras.layers.Add()([x, inputs])  # Residual connection after attention
  ...
  x = tf.keras.layers.Dropout(dropout)(ffn_output)
  x = tf.keras.layers.Add()([x, inputs])  # Residual connection after FFN
  x = tf.keras.layers.LayerNormalization(epsilon=1e-6)(x)

  return x

def build_informer_model(seq_length, n_features, head_size=7, num_heads=4, ff_dim=64, num_encoder_layers=1, dropout=0.1):  # Adjusted parameters
  input_layer = tf.keras.layers.Input(shape=(seq_length, n_features))
  x = input_layer

  # Stack multiple Informer encoder layers
  for _ in range(num_encoder_layers):
    x = informer_encoder(x, seq_length, head_size, num_heads, ff_dim, dropout)
...
if __name__ == '__main__':
  ...
  X_seq, y_seq = create_sequences(X, y_onehot, seq_length)

  # Train-Test Split (80% training, 20% testing)
  X_train, X_test, y_train_onehot, y_test_onehot = train_test_split(X_seq, y_seq, test_size=0.2, shuffle=False)

  # Build Informer model
  model = build_informer_model(seq_length=seq_length, n_features=X_train.shape[2], head_size=7, num_heads=4, ff_dim=64, num_encoder_layers=1, dropout=0.1)
  ...
  # Class weights (adjusted for SMOTE-balanced data)
  class_weights = {
    0: 1.0,  # Hold
    1: 1.0,  # Short
    2: 1.0   # Long
  ...
  lr_scheduler = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_accuracy', factor=0.5, patience=20, min_lr=1e-6, verbose=1)
  ...
  train_indices = df_features_merged.index[seq_length:seq_length + len(y_train_onehot)]
  test_indices = df_features_merged.index[seq_length + len(y_train_onehot):seq_length + len(y_train_onehot) + len(y_test_onehot)]
  ...
  train_profit, train_accuracy = run_bot(df_train, y_train_pred, "Train Dataset", f'{prefix_kline}_closePrice', signal_prob_threshold=0.5)
  test_profit, test_accuracy = run_bot(df_test, y_test_pred, "Test Dataset", f'{prefix_kline}_closePrice', signal_prob_threshold=0.5)
  ...
  # Save scaled dataset
  df_features_merged.to_csv(f'{folder_features_kline}/features_scaled.csv')
  print('\nScaled dataset has been saved.')

  # Save the model and the scaler
  model.save('models/model_kline_informer.keras')
